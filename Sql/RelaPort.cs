...
{
    public class RelaPort
    {
        public DataTable LoaData(string strWhere, string order, string asc, ref int totalProperty, int start, int limit)
        {
            string sql = @"select t1.*,t2.name as DeclPortName,t3.name as inspportname,t4.name as createmanname,t5.name as stopmanname from rela_port t1 left join base_customdistrict
        ...
        {
            string sql = @"select * from rela_port where declport='{0}' and inspport='{1}' and kind =1 " + strWhere;
        ...
        public int insert_relaPackage(JObject json, string stopman)
        {
            ...
            string sql = @"insert into rela_port (id,declport,inspport,createman,stopman,createdate,startdate,enddate,enabled,remark,kind)
                                  ...
                                  to_date('{5}','yyyy-mm-dd hh24:mi:ss'),'{6}','{7}','1')";
        ...
        public int insert_base_alterrecord(JObject json, DataTable dt)
        {
            ...
            sql = String.Format(sql,
                                json.Value<string>("ID"), (int)Base_YearKindEnum.Rela_Port, json_user.GetValue("ID"),
        ...
        public string getChange(DataTable dt, JObject json)
        {
            ...
            {
                str += "报关口岸代码：" + dt.Rows[0]["declport"] + "——>" + json.Value<string>("DECLPORT") + "。";
            }

            if (dt.Rows[0]["inspport"] != json.Value<string>("INSPPORT"))
            {
                str += "报检口岸代码：" + dt.Rows[0]["inspport"] + "——>" + json.Value<string>("INSPPORT") + "。";
            }
