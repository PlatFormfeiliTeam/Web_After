...
{
    public class RelaPackage
    {
        public DataTable LoaData(string strWhere, string order, string asc, ref int totalProperty, int start, int limit)
        {
            string sql = @"select t1.*,t2.name as declpackagename,t3.name as insppackagename,t4.name as createmanname,t5.name as stopmanname from rela_package t1 left join base_packing t2 on
            ...
            sql = Extension.GetPageSql2(sql, "t1.declpackage", "", ref totalProperty, start, limit);
        ...
        {
            string sql = @"select * from rela_package where (declpackage='{0}' or insppackage='{1}') " + strWhere;
        ...
        public int insert_relaPackage(JObject json, string stopman)
        {
            ...
            string sql = @"insert into rela_package (id,declpackage,insppackage,createman,stopman,createdate,startdate,enddate,enabled,remark,yearid)
                                  values(rela_package_id.nextval,'{0}','{1}','{2}','{3}',sysdate,to_date('{4}','yyyy-mm-dd hh24:mi:ss'),
                                  to_date('{5}','yyyy-mm-dd hh24:mi:ss'),'{6}','{7}','')";
            sql = string.Format(sql, json.Value<string>("DECLPACKAGE"), json.Value<string>("INSPPACKAGE"), json_user.GetValue("ID"), stopman,
        ...
        {
            string sql = @"select * from rela_package t1 where t1.id='{0}'";
        ...
        public int update_relaPackage(JObject json, string stopman)
        {
            ...
            string userName = identity.Name;
            JObject json_user = Extension.Get_UserInfo(userName);
            string sql = @"update rela_package set declpackage='{0}',insppackage='{1}',createman='{2}',stopman='{3}',createdate=sysdate,
        ...
        public int insert_base_alterrecord(JObject json, DataTable dt)
        {
            ...
            sql = String.Format(sql,
                                json.Value<string>("ID"), (int)Base_YearKindEnum.Rela_Package, json_user.GetValue("ID"),
        ...
        public string getChange(DataTable dt, JObject json)
        {
            ...
            if (dt.Rows[0]["declpackage"] != json.Value<string>("DECLPACKAGE"))
            {
                str += "报关包装代码：" + dt.Rows[0]["declpackage"] + "——>" + json.Value<string>("DECLPACKAGE") + "。";
            }

            if (dt.Rows[0]["insppackage"] != json.Value<string>("INSPPACKAGE"))
            {
                str += "报检包装代码：" + dt.Rows[0]["insppackage"] + "——>" + json.Value<string>("INSPPACKAGE") + "。";
            }
        ...
        public void insert_rela_package_excel(string DECLPACKAGE, string INSPPACKAGE, string ENABLED, string REMARK, string stopman, string STARTDATE, string ENDDATE)
        {
            FormsIdentity identity = HttpContext.Current.User.Identity as FormsIdentity;
            string userName = identity.Name;
            JObject json_user = Extension.Get_UserInfo(userName);
            string sql = @"insert into rela_package (id,declpackage,insppackage,createman,stopman,createdate,startdate,enddate,enabled,remark,yearid)
                                  values(rela_package_id.nextval,'{0}','{1}','{2}','{3}',sysdate,to_date('{4}','yyyy-mm-dd hh24:mi:ss'),
                                  to_date('{5}','yyyy-mm-dd hh24:mi:ss'),'{6}','{7}','')";
            sql = string.Format(sql, DECLPACKAGE, INSPPACKAGE, json_user.GetValue("ID"), stopman,
