<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="RelaCurrency.aspx.cs" Inherits="Web_After.BasicManager.DataRela.RelaCurrency" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    ...
    <script type="text/javascript">
        ...
        var common_data_currency_decl = [], common_data_currency_insp = [];
        var store_currency_decl, store_currency_insp;

        Ext.onReady(function () {
            Ext.Ajax.request({
                url: 'RelaCurrency.aspx',
                params: { action: 'Ini_Base_Data' },
                type: 'Post',
                success: function (response, option) {
                    var commondata = Ext.decode(response.responseText);
                    common_data_currency_decl = commondata.DECLCURRENCY;//
                    common_data_currency_insp = commondata.INSPCURRENCY;//
                    store_currency_decl = Ext.create('Ext.data.JsonStore', { fields: ['CODE', 'NAME'], data: common_data_currency_decl });
                    store_currency_insp = Ext.create('Ext.data.JsonStore', { fields: ['CODE', 'NAME'], data: common_data_currency_insp });
        ...
        function init_search() {
            var searchCode = Ext.create('Ext.form.field.Text', { id: 'DECLCURRENCYCODE', name: 'DECLCURRENCYCODE', fieldLabel: '报关币制代码' });
            var searchName = Ext.create('Ext.form.field.Text', { id: 'DECLCURRENCYNAME', name: 'DECLCURRENCYNAME', fieldLabel: '报关币制名称' });
        ...
        function gridbind() {
            var store_customer = Ext.create('Ext.data.JsonStore',
                {
                    fields: ['DECLCURRENCY', 'DECLCURRENCYNAME', 'INSPCURRENCY', 'INSPCURRENCYNAME', 'ENABLED', 'STARTDATE', 'CREATEMANNAME',
                        'CREATEDATE', 'STOPMANNAME', 'ENDDATE', 'REMARK', 'ID'],
                    pageSize: 20,
                    proxy: {
                        type: 'ajax',
                        url: 'RelaCurrency.aspx?action=loadData',
                        reader: {
                            root: 'rows',
                            type: 'json',
                            totalProperty: 'total'
            ...
            var gridpanel = Ext.create('Ext.grid.Panel', {
                ...
                columns: [
                    ...
                    { header: '报关币制代码', dataIndex: 'DECLCURRENCY', width: 150 },
                    { header: '报关币制名称', dataIndex: 'DECLCURRENCYNAME', width: 150 },
                    { header: '报检币制代码', dataIndex: 'INSPCURRENCY', width: 150 },
                    { header: '报检币制名称', dataIndex: 'INSPCURRENCYNAME', width: 150 },
        ...
        function form_ini_win() {
            ...
            var store_for_decl = Ext.create('Ext.data.JsonStore', {
                fields: ['CODE', 'NAME'],
                data: common_data_currency_decl
            });

            var store_for_insp = Ext.create('Ext.data.JsonStore', {
                fields: ['CODE', 'NAME'],
                data: common_data_currency_insp
            });
        ...
        function importexcel(action) {
            ...
            var radio_module = Ext.create('Ext.form.RadioGroup', {
                ...
                items: [
                    { boxLabel: "<a href='/FileUpload/rela_currency.xls'><b>模板</b></a>", name: 'RADIO_MODULE', inputValue: '1', checked: true }
                ]
            });
        ...
        function exportdata() {
            var DECLCURRENCYCODE = Ext.getCmp('DECLCURRENCYCODE').getValue();
            var DECLCURRENCYNAME = Ext.getCmp('DECLCURRENCYNAME').getValue();
            var combo_ENABLED_S = Ext.getCmp('combo_ENABLED_S').getValue();
            var path = 'RelaCurrency.aspx?action=export&DECLCURRENCYCODE=' + DECLCURRENCYCODE + '&DECLCURRENCYNAME=' + DECLCURRENCYNAME + '&combo_ENABLED_S=' + combo_ENABLED_S;
