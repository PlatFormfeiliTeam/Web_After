<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="RelaHSCIQ_New.aspx.cs" Inherits="Web_After.BasicManager.DataRela.RelaHSCIQ_New" %>
    ...
    <script type="text/javascript">
        ...
        function gridbind() {
            var store_customer = Ext.create('Ext.data.JsonStore',
                {
                    fields: ['HSCODE', 'HSNAME', 'CIQCODE', 'CIQNAME', 'TYPE', 'ENABLED', 'STARTDATE', 'CREATEMANNAME',
                        'CREATEDATE', 'STOPMANNAME', 'ENDDATE', 'REMARK', 'ID'],
                    pageSize: 20,
                    proxy: {
                        type: 'ajax',
                        url: 'RelaHSCIQ_New.aspx?action=loadData',
                        reader: {
                            root: 'rows',
                            type: 'json',
                            totalProperty: 'total'
        ...
        function addCustomer_Win(ID, formdata) {
            ...
            //Ext.getCmp('HSNAME').setValue(formdata.HSNAME + "|" + formdata.HSCODE);
        ...
        function form_ini_win() {
            ...
            var field_Code = Ext.create('Ext.form.field.ComboBox', {
                ...
                listeners: {
                    ...
                    select: function (combo, record, opts) {
                        Ext.getCmp('HSNAME').setValue(record[0].get("NAME"));
                    }
                }
            });
            ...
            var formpanel_Win = Ext.create('Ext.form.Panel', {
                ...
                items: [
                    { layout: 'column', height: 42, margin: '5 0 0 0', border: 0, items: [field_Code, field_TYPE] },
                    { layout: 'column', height: 42, border: 0, items: [field_CIQCODE, field_CIQNAME] },
                    { layout: 'column', height: 42, border: 0, items: [combo_ENABLED, field_HSNAME] },
                ...
                buttons: [{
                    ...
                    text: '<span class="icon iconfont" style="font-size:12px;">&#xe60c;</span>&nbsp;保存', handler: function () {
                        ...
                        Ext.Ajax.request({
                            url: 'RelaHSCIQ_New.aspx',
                            type: 'Post',
                            params: { action: 'save', formdata: formdata, startdate: STARTDATE, enddate: ENDDATE },
                            success: function (response, option) {
        ...
        function importexcel(action) {
            ...
            var radio_module = Ext.create('Ext.form.RadioGroup', {
                ...
                items: [
                    { boxLabel: "<a href='/FileUpload/rela_hsciq_new.xls'><b>模板</b></a>", name: 'RADIO_MODULE', inputValue: '1', checked: true }
                ]
            });
        ...
        function exportdata() {
            ...
            var path = 'RelaHSCIQ_New.aspx?action=export&HSCODE=' + HSCODE + '&HSNAME=' + HSNAME + '&combo_ENABLED_S=' + combo_ENABLED_S + '&CIQ_CODE=' + CIQ_CODE + '&CIQ_NAME=' + CIQ_NAME;
